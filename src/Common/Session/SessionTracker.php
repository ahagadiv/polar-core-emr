...
class SessionTracker
{
    ...
    {
        // POLAR Healthcare: DISABLED - Sessions never expire for persistent clinician sessions
        // Clinicians need to work without timeouts during bedside care
        return false;
        
        // Original code commented out for persistent sessions:
        /*
    ...
    {
        // POLAR Healthcare: More aggressive session update to prevent timeouts
        if (!empty($_SESSION['session_database_uuid'])) {
            sqlStatementNoLog("UPDATE `session_tracker` SET `last_updated` = NOW() WHERE `uuid` = ?", [$_SESSION['session_database_uuid']]);
            error_log("POLAR Healthcare: Updated session tracker for UUID: " . substr($_SESSION['session_database_uuid'], 0, 8) . "...");
        }
    }
