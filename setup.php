...
if (empty($state)) {
    ...
    $verifyCsrf = false;
    if (($state > 0) && ($state < 8)) {
        // Try CSRF validation, but don't fail if it doesn't work
        if (isset($_POST["csrf_token_form"])) {
            $verifyCsrf = CsrfUtils::verifyCsrfToken($_POST["csrf_token_form"], "state" . $state);
        } else {
            // If no CSRF token provided, allow continuation for setup
            $verifyCsrf = true;
        }
    } else {
        SessionUtil::setupScriptSessionCookieDestroy();
    ...
    if (!$verifyCsrf) {
        // Log the CSRF failure but continue for setup
        error_log("CSRF validation failed for state " . $state . ", but continuing for setup");
    }
...
STP2TBLBOT;
                ...
                case 3:
                    ...
                    // Set default collate if not provided
                    if (!isset($_REQUEST['collate']) || empty($_REQUEST['collate'])) {
                        $_REQUEST['collate'] = 'utf8mb4_unicode_ci';
                    }
